...
// src/index.ts
var import_compiler_sfc6 = __toModule(require("@vue/compiler-sfc"));
...
function getDescriptor(filename, root, isProduction, createIfNotFound = true) {
  ...
  if (createIfNotFound) {
    const { descriptor, errors } = createDescriptor(filename, import_fs.default.readFileSync(filename, "utf-8"), root, isProduction);
    if (errors) {
      throw errors[0];
    }
    return descriptor;
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  if (((_e = descriptor.script) == null ? void 0 : _e.lang) === "ts" || ((_f = descriptor.scriptSetup) == null ? void 0 : _f.lang) === "ts") {
...
import ${JSON.stringify(styleRequest)}`;
    ...
    if (asCustomElement) {
      stylesCode += `
_sfc_main.styles = [${descriptor.styles.map((_, i) => `_style_${i}`).join(",")}]`;
    }
  }
  return stylesCode;
}
...
function vuePlugin(rawOptions = {}) {
  ...
  const canUseRefTransform = typeof import_compiler_sfc6.shouldTransformRef === "function";
  ...
  return {
    ...
    load(id, ssr = !!options.ssr) {
      ...
      if (query.vue) {
        ...
        const descriptor = getDescriptor(filename, options.root, options.isProduction);
        let block;
    ...
    transform(code, id, ssr = !!options.ssr) {
      ...
      if (!filter(filename) && !query.vue) {
        if (!query.vue && refTransformFilter(filename)) {
          ...
          } else if ((0, import_compiler_sfc6.shouldTransformRef)(code)) {
            return (0, import_compiler_sfc6.transformRef)(code, {
              filename,
              sourceMap: true
            });
          }
