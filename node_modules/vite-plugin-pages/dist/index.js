...
// node_modules/.pnpm/tsup@5.1.0_ts-node@10.2.1+typescript@4.4.3/node_modules/tsup/assets/cjs_shims.js
...
async function parseSFC(code) {
  try {
    const { parse: parse2 } = await Promise.resolve().then(() => __toModule(require("@vue/compiler-sfc")));
    return parse2(code, {
      pad: "space"
    }).descriptor;
  } catch {
...
function replaceSquareBrackets(bundle) {
  const files = Object.keys(bundle).map((i) => (0, import_path.basename)(i));
...
function getPageDirs(pageDirOptions, root, exclude) {
  const dirs = import_fast_glob.default.sync(pageDirOptions.dir, {
...
async function resolvePages(options) {
  ...
  const pageDirFiles = dirs.map((pageDir) => {
    const pagePath = (0, import_utils.slash)((0, import_path2.resolve)(options.root, pageDir.dir));
...
async function setPage(pages, pageDir, file, options) {
  const component = (0, import_utils.slash)((0, import_path2.join)(pageDir.dir, file));
  const filepath = (0, import_utils.slash)((0, import_path2.resolve)(options.root, component));
  const extension = (0, import_path2.extname)(file).slice(1);
  const customBlock = ["vue", "md"].includes(extension) ? await getRouteBlock(filepath, options) : null;
  pages.set(filepath, {
    dir: pageDir.dir,
    route: (0, import_utils.slash)((0, import_path2.join)(pageDir.baseRoute, file.replace(options.extensionsRE, ""))),
    extension,
    filepath,
    component,
    customBlock
...
function generateRoutes(pages, options) {
  ...
  const allRoute = finalRoutes.find((i) => {
    return isCatchAllRoute((0, import_path3.parse)(i.component).name, nuxtStyle);
  });
  if (allRoute) {
    finalRoutes = finalRoutes.filter((i) => !isCatchAllRoute((0, import_path3.parse)(i.component).name, nuxtStyle));
...
function resolvePageDirs(pagesDir, root, exclude) {
  ...
  return pagesDir.flatMap((pagesDir2) => {
    ...
    option.dir = (0, import_utils.slash)((0, import_path4.resolve)(root, option.dir)).replace(`${root}/`, "");
...
function handleHMR(server, pages, options, clearRoutes) {
  ...
  watcher.on("add", async (file) => {
    const path = (0, import_utils.slash)(file);
