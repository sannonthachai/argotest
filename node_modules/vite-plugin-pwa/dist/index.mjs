...
async function configureStaticAssets(resolvedVitePWAOptions, viteConfig) {
  ...
  if (globs.length > 0) {
    let assets = await fg(globs.map((g) => {
...
function resolveSwPaths(injectManifest2, root, srcDir, outDir, filename) {
  ...
  if (injectManifest2 && extname(filename) === ".ts" && fs3.existsSync(swSrc)) {
...
function VitePWA(userOptions = {}) {
  ...
  function _generateBundle(bundle = {}) {
    ...
    if (options.injectRegister === "script" && !existsSync(resolve3(viteConfig.publicDir, FILE_SW_REGISTER))) {
